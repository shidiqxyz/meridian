...
import { discoverPools, getPoolDetail } from "../src/tools/screening.js";

async function main() {
  console.log("=== Testing Pool Discovery API ===\n");

  console.log("Fetching top 10 pools...");
  const top = await discoverPools({ page_size: 10 });
  console.log(`Found ${top.total} total pools, showing ${top.pools?.length ?? 0}`);

  if (top.pools && top.pools.length > 0) {
  ...
  const trending = await discoverPools({ page_size: 5 });
  console.log(`Found ${trending.pools?.length ?? 0} trending pools`);
