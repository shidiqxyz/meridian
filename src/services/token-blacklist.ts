 ...
 */
export function addToBlacklist({ mint, symbol, reason }: AddToBlacklistParams): { error?: string; already_blacklisted?: boolean; blacklisted?: boolean; mint?: string; symbol?: string; reason?: string } {
 ...
 */
export function removeFromBlacklist({ mint }: RemoveFromBlacklistParams): { error?: string; removed?: boolean; mint?: string; was?: BlacklistEntry } {
  if (!mint) return { error: "mint required" };
