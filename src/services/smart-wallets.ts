...
interface AddSmartWalletParams {
  name: string;
  address: string;
  category?: string;
  type?: string;
...
interface RemoveSmartWalletParams {
  address: string;
}

interface CheckSmartWalletsParams {
  pool_address: string;
}
...
export function addSmartWallet({ name, address, category = "alpha", type = "lp" }: AddSmartWalletParams): { success: boolean; error?: string; wallet?: SmartWallet } {
  ...
  const wallet: SmartWallet = { name, address, category, type, addedAt: new Date().toISOString() };
...
export function removeSmartWallet({ address }: RemoveSmartWalletParams): { success: boolean; error?: string; removed?: string } {
  const data = loadWallets();
...
const _cache = new Map<string, { positions: any[]; fetchedAt: number }>();
...
export async function checkSmartWalletsOnPool({ pool_address }: CheckSmartWalletsParams): Promise<CheckResult> {
  ...
  const inPool = results
    .filter((r) => r.positions.some((p: any) => p.pool === pool_address))
