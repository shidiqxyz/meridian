...
async function postTelegram(method: string, body: Record<string, unknown>): Promise<TelegramResponse | null> {
...
async function postTelegramRaw(method: string, body: Record<string, unknown>): Promise<TelegramResponse | null> {
...
export async function sendMessage(text: string): Promise<TelegramResponse | null> {
  if (!TOKEN || !chatId) return null;
  return postTelegram("sendMessage", { text: String(text).slice(0, 4096) });
}

export async function sendMessageWithButtons(text: string, inlineKeyboard: unknown): Promise<TelegramResponse | null> {
  if (!TOKEN || !chatId) return null;
...
export async function sendHTML(html: string): Promise<TelegramResponse | null> {
  if (!TOKEN || !chatId) return null;
  return postTelegram("sendMessage", { text: html.slice(0, 4096), parse_mode: "HTML" });
}

export async function editMessage(text: string, messageId: number): Promise<TelegramResponse | null> {
...
export async function editMessageWithButtons(text: string, messageId: number, inlineKeyboard: unknown): Promise<TelegramResponse | null> {
  if (!TOKEN || !chatId || !messageId) return null;
  return postTelegram("editMessageText", {
    message_id: messageId,
    text: String(text).slice(0, 4096),
...
export async function answerCallbackQuery(callbackQueryId: string, text = ""): Promise<TelegramResponse | null> {
...
export async function createLiveMessage(title: string, intro = "Starting..."): Promise<{
  ...
  async function upsertToolLine(name: string, icon: string, suffix = ""): Promise<void> {
    ...
    const idx = state.toolLines.findIndex((entry: string) => entry.includes(` ${label}`));
  ...
  return {
    ...
    async toolFinish(name: string, result: any, success: boolean) {
      const icon = success ? "✅" : "❌";
...
export async function notifyDeploy({ pair, amountSol, position, tx, priceRange, rangeCoverage, binStep, baseFee }: DeployNotification): Promise<void> {
...
export async function notifyClose({ pair, pnlUsd, pnlPct }: CloseNotification): Promise<void> {
  if (hasActiveLiveMessage()) return;
  const sign = (pnlUsd ?? 0) >= 0 ? "+" : "";
  await sendHTML(
    "🔒 <b>Closed</b> " + pair + "\n" +
    `PnL: ${sign}$${(pnlUsd ?? 0).toFixed(2)} (${(pnlPct ?? 0).toFixed(2)}%)`
  );
}
...
export async function notifySwap({ inputSymbol, outputSymbol, amountIn, amountOut, tx }: SwapNotification): Promise<void> {
  if (hasActiveLiveMessage()) return;
  await sendHTML(
...
interface OorNotification {
  pair: string;
  minutesOOR: number;
}

export async function notifyOutOfRange({ pair, minutesOOR }: OorNotification): Promise<void> {
  if (hasActiveLiveMessage()) return;
  await sendHTML(
