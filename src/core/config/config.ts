...
const u: UserConfig = fs.existsSync(USER_CONFIG_PATH)
...
if (u.walletKey) {
  throw new Error(
    "Plaintext walletKey is not allowed in user-config.json. " +
    "Use envcrypt to encrypt sensitive values in .env instead."
  );
}
...
function nonEmptyString(...values: (string | null | undefined)[]): string | null {
...
/**
 * Reload user-config.json and apply updated screening thresholds.
 */
export function reloadScreeningThresholds(): void {
  if (!fs.existsSync(USER_CONFIG_PATH)) return;
  try {
    const fresh: UserConfig = JSON.parse(fs.readFileSync(USER_CONFIG_PATH, "utf8"));
