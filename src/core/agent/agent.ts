...
import { getStateSummary } from "../../core/state/state.js";
import { getLessonsForPrompt, getPerformanceSummary } from "../../core/state/lessons.js";
import { getDecisionSummary } from "../../core/state/decision-log.js";
...
const DEFAULT_MODEL = process.env.LLM_MODEL || "deepseek/deepseek-v4-flash:nitro";
...
function shouldRequireRealToolUse(goal: string, agentType: string, interactive = false): boolean {
...
type ToolStartCallback = (data: { name: string; args: Record<string, unknown>; step: number }) => void | Promise<void>;
type ToolFinishCallback = (data: { name: string; args: Record<string, unknown>; result: unknown; success: boolean; step: number }) => void | Promise<void>;
