...
interface Portfolio {
  ...
  sol_usd: number;
  usdc: number;
  tokens: Array<{ mint: string; symbol: string; balance: number; usd_value: number }>;
  total_usd: number;
}
...
export function buildSystemPrompt(
  agentType: string,
  portfolio: Portfolio,
  positions: { positions: Position[]; total_positions: number },
  stateSummary: StateSummary | null = null,
  lessons: string | null = null,
  perfSummary: PerfSummary | null = null,
...
Call pickBestCandidate() to get the single best pool (pre-scored and filtered). Then do quick due diligence: check_smart_wallets_on_pool, get_token_narrative, get_token_info. If it passes, call deploy_position immediately.
...
- rugpull flag from OKX → major negative, default to SKIP; only override if smart wallets are present and conviction is otherwise high
- wash trading flag from OKX → treat as disqualifying even if other metrics look attractive
- PVP symbol conflict → skip unless exceptional
- no narrative + no smart wallets → skip

NARRATIVE QUALITY:
- GOOD: specific origin, real event, viral moment, active community
- BAD: generic hype ("next 100x") with no identifiable subject
- Smart wallets present → can override weak narrative

POOL MEMORY: Past losses or problems → strong skip signal.

DEPLOY RULES:
- Use the deploy amount from the goal EXACTLY. Do NOT default to smaller number.
