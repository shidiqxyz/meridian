...
import type { PerformanceRecord, Lesson, LessonsData, EvolveResult } from "../types/state";
...
function derivLesson(perf: PerformanceRecord): Lesson | null {
  const tags: string[] = [];
...
export function evolveThresholds(perfData: PerformanceRecord[], config: Config): EvolveResult | null {
  ...
  data.lessons.push({
    ...
    confidence: 1,
    context: "Automatic threshold evolution",
    created_at: new Date().toISOString(),
  });
  save(data);
...
export function addLesson(
  rule: string,
  tags: string[] = [],
  { pinned = false, role = null }: { pinned?: boolean; role?: string | null } = {},
): void {
...
export function listLessons(
  { role = null, pinned = null, tag = null, limit = 30 }: { role?: string | null; pinned?: boolean | null; tag?: string | null; limit?: number } = {},
): {
  total: number;
  lessons: Array<{
    id: number;
...
export function getLessonsForPrompt(opts: number | { agentType?: string; maxLessons?: number } = {}): string | null {
  if (typeof opts === "number") opts = { maxLessons: opts };

  const { agentType = "GENERAL", maxLessons } = opts as { agentType?: string; maxLessons?: number };
...
export function getPerformanceHistory({ hours = 24, limit = 50 } = {}): {
  ...
  if (p.length === 0) return { positions: [], count: 0, hours, total_pnl_usd: 0, win_rate_pct: null };
