 ...
 */
export function recalculateWeights(perfData: PerfRecord[], cfg: { darwin?: DarwinConfig } = {}): { changes: Array<{ signal: string; from: number; to: number; lift: number; action: string }>; weights: Record<string, number> } {
  ...
  let weights = data.weights || { ...DEFAULT_WEIGHTS };
  ...
  const changes: Array<{ signal: string; from: number; to: number; lift: number; action: string }> = [];
...
function computeLift(signal: string, wins: PerfRecord[], losses: PerfRecord[], minSamples: number): number | null {
...
function computeNumericLift(signal: string, wins: PerfRecord[], losses: PerfRecord[], minSamples: number): number | null {
...
function computeBooleanLift(signal: string, wins: PerfRecord[], losses: PerfRecord[], minSamples: number): number | null {
  const allEntries = [
...
function computeCategoricalLift(signal: string, wins: PerfRecord[], losses: PerfRecord[], minSamples: number): number | null {
  const allEntries = [
    ...wins.map((w) => ({ w: true, snap: w })),
    ...losses.map((l) => ({ w: false, snap: l })),
  ];
...
function extractNumeric(signal: string, entries: PerfRecord[]): number[] {
  const vals: number[] = [];
