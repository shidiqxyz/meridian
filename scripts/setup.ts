...
import "../src/core/config/envcrypt.js";
...
async function askNum(question: string, defaultVal: number, { min, max }: { min?: number; max?: number } = {}): Promise<number> {
  while (true) {
...
async function askBool(question: string, defaultVal: boolean): Promise<boolean> {
  while (true) {
...
function askChoice<T>(question: string, choices: T[], getLabel: (c: T) => string): Promise<T> {
  return new Promise(async (resolve) => {
    const labels = choices.map((c, i) => `  ${i + 1}. ${getLabel(c)}`).join("\n");
    while (true) {
      console.log(`\n${question}`);
      console.log(labels);
...
const e  = (key: string, fallback: any) => existingConfig[key] ?? fallback;
const ev = (key: string, fallback: any) => existingEnv[key] ?? fallback;
...
const alreadySet = (val: string | undefined | boolean) => val ? "*** (already set — Enter to keep)" : "";
...
const presetChoice = await askChoice("Select a risk preset:", [
  ...
  { label: "Moderate — 4h timeframe, balanced risk/reward. Recommended for most users.", key: "moderate" },
  { label: "Safe     — 24h timeframe, stable pools only, avoids pumps. Lower yield, lower risk.", key: "safe" },
  { label: "Custom   — Configure every setting manually", key: "custom" },
], (c: PresetChoice) => c.label);

const preset = presetChoice.key === "custom" ? null : PRESETS[presetChoice.key];
const p = (key: string, fallback: any) => preset?.[key] ?? e(key, fallback);
...
const providerChoice = await askChoice("Select LLM provider:", LLM_PROVIDERS, (p) => p.label);
...
if (provider.key === "local" || provider.key === "custom") {
  llmBaseUrl = await ask("Base URL", e("llmBaseUrl", provider.baseUrl || "http://localhost:1234/v1")) || "http://localhost:1234/v1";
}
...
// Write .env
const isKept = (val: string | undefined) => !val || val.startsWith("***");
